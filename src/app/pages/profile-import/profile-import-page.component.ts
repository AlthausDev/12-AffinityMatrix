...
import { PortableProfileV1 } from '../../../application/profile/portable-profile';
import { decodeProfileCode } from '../../../infrastructure/serialization/profile-codec';
  ...
  template: `
    <main class="page narrow-page">
      ...
      <section class="panel form-grid">
        ...
        @if (codeError()) {
          <p class="alert" role="alert">{{ codeError() }}</p>
        }
      ...
      @if (preview(); as portable) {
        <section class="panel import-preview" aria-labelledby="import-preview-title">
          <div>
            <p class="eyebrow">Valid profile</p>
            <h2 id="import-preview-title">{{ portable.metadata.alias || 'Untitled profile' }}</h2>
            <p class="muted">{{ answerCount(portable) }} answered roles</p>
          </div>
          ...
          <div class="form-actions import-actions">
            <button class="button secondary" type="button" disabled>Compare without saving</button>
            <button class="button" type="button" (click)="saveProfile()">Save in this browser</button>
          </div>
        </section>
      }
    </main>
...
export class ProfileImportPageComponent {
  ...
  readonly preview = signal<PortableProfileV1 | null>(null);
  ...
  inspectCode(): void {
    try {
      this.preview.set(decodeProfileCode(this.code()));
      this.codeError.set('');
    } catch (error: unknown) {
      this.preview.set(null);
  ...
  answerCount(portable: PortableProfileV1): number {
    return Object.keys(portable.answers).length;
  }
}
