...
@Component({
      ...
      @if (profile(); as currentProfile) {
        <header class="page-header dashboard-header">
          ...
          <div class="dashboard-header-actions">
            ...
            <a class="button secondary settings-button" [routerLink]="['/profiles', currentProfile.id, 'settings']">
        ...
        <section class="action-grid" [attr.aria-label]="i18n.t('dashboard.actionsLabel')">
          <article class="action-card">
            <div>
              ...
              <h2>{{ i18n.t(savedAnswerCount() > 0 ? 'dashboard.questionnaire.continueTitle' : 'dashboard.questionnaire.startTitle') }}</h2>
              <p class="muted">{{ i18n.t('dashboard.questionnaire.description') }}</p>
            </div>
            <a class="button" [routerLink]="['/profiles', currentProfile.id, 'questionnaire']">{{ i18n.t(savedAnswerCount() > 0 ? 'dashboard.questionnaire.continue' : 'dashboard.questionnaire.start') }}</a>
          </article>
          <article class="action-card">
            <div>
        ...
        <section class="panel profile-status" aria-labelledby="profile-status-title">
          <header class="status-heading">
            <div>
              <p class="eyebrow">{{ i18n.t('dashboard.status.eyebrow') }}</p>
              <h2 id="profile-status-title">{{ i18n.t('dashboard.status.title') }}</h2>
            </div>
            @if (totalQuestions() > 0) {
              <div class="overall-percentage">
                <strong>{{ completionPercentage() }}%</strong>
                <span>{{ i18n.t('dashboard.status.overallProgress') }}</span>
              </div>
            }
          </header>
          ...
          @if (totalQuestions() > 0) {
            <div class="overall-progress-block">
              <div class="progress-copy">
                <strong>{{ i18n.t('dashboard.status.overallProgress') }}</strong>
                <span class="muted">{{ i18n.t('dashboard.status.visibleAnswered', { answered: totalAnswered(), total: totalQuestions() }) }}</span>
              </div>
              <app-completion-progress [value]="completionPercentage()" />
            </div>
          } @else if (catalogueStore.loading()) {
            <p class="muted status-message">{{ i18n.t('dashboard.status.catalogueLoading') }}</p>
          } @else if (catalogueStore.error()) {
            <p class="muted status-message">{{ i18n.t('dashboard.status.catalogueUnavailable') }}</p>
          }

          <dl class="status-list">
            <div>
              <dt>{{ i18n.t('dashboard.status.categoriesComplete') }}</dt>
              <dd>{{ completedCategories() }} / {{ totalCategories() }}</dd>
            </div>
            <div>
              <dt>{{ i18n.t('dashboard.status.savedAnswers') }}</dt>
              <dd>{{ savedAnswerCount() }}</dd>
            </div>
            <div>
              <dt>{{ i18n.t('dashboard.status.lastUpdated') }}</dt>
              <dd>{{ updatedAtLabel(currentProfile.updatedAt) }}</dd>
            </div>
            <div>
              <dt>{{ i18n.t('dashboard.status.questionFilter') }}</dt>
              <dd>{{ i18n.t(currentProfile.settings.filterQuestionnaireByMetadata ? 'dashboard.status.enabled' : 'dashboard.status.disabled') }}</dd>
            </div>
            <div>
              <dt>{{ i18n.t('dashboard.status.catalogue') }}</dt>
              <dd>v{{ currentProfile.catalogueVersion }}</dd>
            </div>
            <div>
              <dt>{{ i18n.t('dashboard.status.internalVersion') }}</dt>
              <dd>v{{ currentProfile.schemaVersion }}</dd>
            </div>
            <div class="storage-status">
              <dt>{{ i18n.t('dashboard.status.storage') }}</dt>
              <dd>{{ i18n.t('dashboard.status.localNotEncrypted') }}</dd>
            </div>
          </dl>
              ...
              <div class="category-progress-list">
                @for (summary of categorySummaries(); track summary.category.id) {
                  <div class="category-progress-row">
                    <div class="category-progress-copy">
                      <strong>{{ catalogueText.categoryLabel(summary.category) }}</strong>
                      <span class="muted">{{ i18n.t('dashboard.status.categoryValue', { answered: summary.answered, total: summary.total, percentage: summary.completionPercentage }) }}</span>
                    </div>
  ...
  styles: `
    ...
    @media (max-width: 720px) {
      .dashboard-header-actions, .status-heading, .progress-copy, .category-progress-copy { align-items: flex-start; flex-direction: column; }
      .dashboard-header-actions { width: 100%; }
      .settings-button { width: 100%; }
      .overall-percentage { text-align: left; }
      .category-progress-list { grid-template-columns: 1fr; }
...
export class ProfileDashboardPageComponent {
  ...
  readonly categorySummaries = computed(() => {
    ...
    return profile && snapshot ? this.questionnaireService.getCategorySummaries(snapshot, profile, false) : [];
  });
